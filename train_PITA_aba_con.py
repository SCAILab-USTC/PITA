...
################################################################
# configs python train_auto_one.py --gpu 7
...
parser.add_argument('--time_cut',type=int,default=5)
parser.add_argument('--seed',type=int,default=45)
parser.add_argument('--use_full_test' ,type = bool, default=False)
...
if args.use_full_test == True:
    ...
    test_datasets = [MixedTemporalDataset(test_path, res=args.res, n_channels = train_dataset.n_channels,t_in = args.T_in, t_ar=10, normalize=False, train=True) for i, test_path in enumerate(test_paths)]
...
log_path = './logs_Phy_Data/' + time.strftime('%m%d_%H_%M_%S') + comment if len(args.log_path)==0  else os.path.join('./logs',args.log_path + comment)
...
for ep in range(args.epochs):
    ...
    for xx, yy, msk, cls in train_loader:
        ...
        for t in range(0, yy.shape[-2], args.T_bundle):
            ...
            # im, _ = model(xx)
            # xx_copy =  xx.clone().detach()
            im, _ = model(xx)
            im_copy = im.clone()    #.detach()
            y_copy = y.clone()     
            ...
            num_nan_lib_poly = torch.isnan(lib_poly).sum().item()
            num_inf_lib_poly = torch.isinf(lib_poly).sum().item()
            num_nan_U_poly = torch.isnan(U).sum().item()
            num_inf_U_poly = torch.isinf(U).sum().item()
            num_nan_U_t = torch.isnan(U_t).sum().item()
            num_inf_U_t = torch.isinf(U_t).sum().item()
            if num_nan_lib_poly+num_inf_lib_poly+num_nan_U_poly+num_inf_U_poly+num_nan_U_t+num_inf_U_t>0:
                print(f"Detected NaN or Inf values: "
                            f"lib_poly -> NaN: {num_nan_lib_poly}, Inf: {num_inf_lib_poly}; "
                            f"U -> NaN: {num_nan_U_poly}, Inf: {num_inf_U_poly}; "
                            f"U_t -> NaN: {num_nan_U_t}, Inf: {num_inf_U_t}. "
                            f"Replacing with safe values.")

                lib_poly = torch.nan_to_num(lib_poly, nan=0.0, posinf=torch.finfo(lib_poly.dtype).max, neginf=torch.finfo(lib_poly.dtype).min)
                U_t = torch.nan_to_num(U_t, nan=0.0, posinf=torch.finfo(U_t.dtype).max, neginf=torch.finfo(U_t.dtype).min)
        ...
        total_loss = awl(loss_, coefficient_loss_)
    ...
    print('epoch {}, time {:.5f}, lr {:.2e}, train l2 step {:.5f} train l2 full {:.5f}, test l2 step {} test l2 full {}, time train avg {:.5f} load avg {:.5f} test {:.5f}'\
        .format(ep, t2 - t1, lr,train_l2_step_avg, train_l2_full_avg,', \
