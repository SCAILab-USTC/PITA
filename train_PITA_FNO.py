...
################################################################
# configs
# python train_temporal.py --gpu 7 
################################################################
...
# 'cfdbench'
parser.add_argument('--train_paths',nargs='+', type=str, default=['ns2d_pdb_M1e-1_eta1e-1_zeta1e-1'])
parser.add_argument('--test_paths',nargs='+',type=str, default=['ns2d_pdb_M1e-1_eta1e-1_zeta1e-1'])
# 差一个 ns2d_pdb_M1_eta1e-1_zeta1e-1
...
parser.add_argument('--des',type=str, default='cycle')
...
if args.model == "FNO":
    model = FNO2d(
                ...
                n_cls=len(args.train_paths)).to(device)


elif args.model == 'DPOT':
    model = DPOTNet(img_size=args.res, patch_size=args.patch_size, 
                    in_channels=train_dataset.n_channels, 
                    in_timesteps = args.T_in, out_timesteps = args.T_bundle, 
...
log_path = './logs_FNO_T_ar=1/' + time.strftime('%m%d_%H_%M_%S') + f'{args.train_paths[0]}' + comment if len(args.log_path)==0  else os.path.join('./logs',args.log_path + comment)
...
for ep in range(args.epochs):
    ...
    for xx, yy, msk, cls in train_loader:
        ...
        for t in range(0, yy.shape[-2], args.T_bundle):
            ...
            #  im.shape = torch.Size([20, 8, 8, 1, 1]) y.shape = torch.Size([20, 64, 64, 1, 1])
    ...
    print(
        'epoch {}, time {:.5f}, lr {:.2e},'.format(ep, t2 - t1, lr),
        'train l2 step {:.5f},'.format(train_l2_step_avg),
        'train l2 full {:.5f},'.format(train_l2_full_avg),
        'test10 l2 step [{}],'.format(', '.join(['{:.5f}'.format(val) for val in test_l2_steps])),
        'test10 l2 full [{}],'.format(', '.join(['{:.5f}'.format(val) for val in test_l2_fulls])),
        'testfull l2 step ({}),'.format(', '.join(['{:.5f}'.format(val) for val in test_l2_steps_full_data])),
        'testfull l2 full ({}),'.format(', '.join(['{:.5f}'.format(val) for val in test_l2_fulls_full_data])),        
        'cls acc {:.5f},'.format(cls_acc),
        'time train avg {:.5f},'.format(t_train / len(train_loader)),
        'load avg {:.5f},'.format(t_load / len(train_loader)),
        'test {:.5f}'.format(t_test)
    )
