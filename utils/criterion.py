...
# myloss = SimpleLpLoss(size_average=False, reduction=True,return_comps = False)
...
class SimpleLpLoss(_WeightedLoss):
    ...
    def forward(self, x, y, mask=None):
        ...
        # print(f'diff_norms = {diff_norms}\t y_norms = {y_norms}')
        # print(f'diff_norms.shape = {diff_norms.shape}\t y_norms.shape = {y_norms.shape}')
        if self.reduction:
            if self.size_average:
                    return torch.mean(diff_norms/y_norms)          ## deprecated
            else:
                # print(f'msk_channels = {msk_channels}')
