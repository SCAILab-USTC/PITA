...
class MixedTemporalDataset(Dataset):
    # _num_datasets = 0
    # _num_channels = 0
    def __init__(self, data_names, n_list = None, res = 128,t_in = 10, t_ar = 1, n_channels = None, normalize=False,train=True,data_weights=None, use_full_data=False):
        '''
        ...
        self.t_tests_10 = [10] * len(self.data_names)

        self.downsamples = [DATASET_DICT[name]['downsample'] for name in self.data_names]
        # self.n_channels = min (1 , max([DATASET_DICT[name]['n_channels'] for name in self.data_names]) if n_channels is None else n_channels )
        ...
        for name in self.data_names:
            if DATASET_DICT[name]['scatter_storage']:
                ...
                print(DATASET_DICT[name]['train_path'] if train else DATASET_DICT[name]['test_path'])
...
# xx=[0:T_in]; yy=[T_in+\delta t]
    ...
    def __getitem__(self, idx):
        ...
        # t_0 = time.time()
        # 打印当前的数据集索引和数据索引
        # print(f"Accessing dataset {self.data_names[dataset_idx]} at index {data_idx}")
        ...
        if self.train==True:  ## sample [0, t_in] and [t_in, t_in+ t_ar] for training ,trucated if too long
        ...
        elif self.train==False and self.use_full_data==True: ## test datasets returns full trajectory
            start_idx = 0
        ...
        elif self.train==False and self.use_full_data==False:
            start_idx = 0
            x, y = sample[..., start_idx:start_idx + self.t_in,:], sample[..., self.t_in:self.t_in + self.t_tests_10[dataset_idx],:]
            # msk = msk[..., self.t_in:self.t_in + self.t_tests[dataset_idx],:]
            msk = self.get_target_mask(sample, orig_size)        
        if self.normalize:
        ...
        # 这是输入数据（输入时间步长）的张量，经过了数据填充、采样以及可选的归一化处理。
        # 形状通常为 (H, W, T_in, C)，其中 H 和 W 是空间分辨率，T_in 是时间步长，C 是通道数。
        # y (Tensor):

        # 这是目标数据（预测时间步长）的张量，用于训练模型进行下一时间步的预测。
        # 形状通常为 (H, W, T_ar, C)，其中 T_ar 是自动回归预训练的步长，表示模型需要预测的时间步长。
        # msk (Tensor):

        # 这是目标掩码，用于在训练和评估过程中掩盖掉不需要的部分。它可以帮助模型专注于特定分辨率或时间步长的数据。
        # 形状通常为 (H, W, 1, C)，即和输入数据的空间分辨率和通道数匹配，但时间步长被设置为 1。
        # idx_cls (Tensor):

        # 这是一个长整型张量，用于表示当前数据所属的数据集索引（在给定的数据集列表中的相对索引）。
        # 形状为 (1,)，内容为当前数据所在的数据集的索引。
