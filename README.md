...
- (1) The pretrained PDE model takes the initial temporal states $\left\{\boldsymbol{u}_ t\right\}_ {t=1}^{T_ {in}}$ as input and predicts future states $\left\{\hat{\boldsymbol{u}}_ t\right\}_ {t=T_{in}+1}^{{T_{in}+T_{ar}}}$ as output in an auto-regressive manner;
