...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, out_dim=128,act='gelu'):
        ...
        self.act = ACTIVATION[act]
        print ( f" nn.Conv2d.in_chans = {in_chans}")
        print ( f" nn.Conv2d.embed_dim = {embed_dim}")
        print ( f" nn.Conv2d.kernel_size = {patch_size}")
        print ( f" nn.Conv2d.stride = {patch_size}")
        ...
        # 创建一个Conv2d层
        conv_layer = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        # 打印权重的形状
    ...
    def forward(self, x):
        ...
        # print ( f"before proj x.shape = {x.shape}")
...
class DPOTNet(nn.Module):
    def __init__(self, img_size=224, 
        ...
        self.act = ACTIVATION[act]
        print ( f"in_chans = {in_channels + 3}")
